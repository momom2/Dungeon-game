...
def test_out_of_bounds_returns_bedrock():
    ...
    assert grid.get(0, 0, -1) == VOXEL_AIR  # z<0 with valid x,y = open sky
...
# ---------------------------------------------------------------------------
# VoxelGrid.get() boundary behavior (from test_rendering_fixes.py)
# ---------------------------------------------------------------------------


class TestGetAboveSurface:
    """VoxelGrid.get() boundary behavior for z<0."""

    def test_above_surface_returns_air(self):
        """z<0 with valid x,y should return VOXEL_AIR (open sky)."""
        grid = VoxelGrid(width=8, depth=8, height=4)
        assert grid.get(0, 0, -1) == VOXEL_AIR
        assert grid.get(3, 5, -1) == VOXEL_AIR
        assert grid.get(7, 7, -10) == VOXEL_AIR  # Any negative z

    def test_above_surface_oob_xy_returns_bedrock(self):
        """z<0 with invalid x or y should still return VOXEL_BEDROCK."""
        grid = VoxelGrid(width=8, depth=8, height=4)
        assert grid.get(-1, 0, -1) == VOXEL_BEDROCK  # x out of bounds
        assert grid.get(0, -1, -1) == VOXEL_BEDROCK  # y out of bounds
        assert grid.get(8, 0, -1) == VOXEL_BEDROCK   # x out of bounds
        assert grid.get(0, 8, -1) == VOXEL_BEDROCK   # y out of bounds

    def test_below_grid_returns_bedrock(self):
        """z>=height should return VOXEL_BEDROCK (unchanged)."""
        grid = VoxelGrid(width=8, depth=8, height=4)
        assert grid.get(0, 0, 4) == VOXEL_BEDROCK
        assert grid.get(3, 3, 100) == VOXEL_BEDROCK

    def test_surface_block_top_face_gets_air_neighbor(self):
        """Stone at z=0 should have air neighbor above (z=-1) for top face."""
        grid = VoxelGrid(width=4, depth=4, height=4)
        grid.grid[2, 2, 0] = VOXEL_STONE
        # The top face direction is (0, 0, -1) -> neighbor at z=-1
        neighbor = grid.get(2, 2, -1)
