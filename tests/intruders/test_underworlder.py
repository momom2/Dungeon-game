...
class TestUnderworldSpawning:
    ...
    def test_spawn_position_at_deep_edge(self):
        ...
        # Carve air at edges for depth z=UNDERWORLD_SPAWN_Z_MIN
        grid.grid[0, 0, UNDERWORLD_SPAWN_Z_MIN] = VOXEL_AIR
        grid.grid[0, 1, UNDERWORLD_SPAWN_Z_MIN] = VOXEL_AIR
    ...
    def test_spawn_timer_uses_underworld_interval(self):
        ...
        grid.grid[0, 0, UNDERWORLD_SPAWN_Z_MIN] = VOXEL_AIR
        grid.grid[:, :, SURFACE_Z] = VOXEL_AIR  # Surface air for surface spawning
    ...
    def test_spawn_at_interval(self):
        ai, bus, grid, core, rng = _make_ai()
        ai.spawning_enabled = True
        # Carve air at edges for spawning (z=17 is within underworld spawn range)
        for x in range(grid.width):
            grid.grid[x, 0, 17] = VOXEL_AIR
            grid.grid[x, grid.depth - 1, 17] = VOXEL_AIR
        for y in range(grid.depth):
            grid.grid[0, y, 17] = VOXEL_AIR
            grid.grid[grid.width - 1, y, 17] = VOXEL_AIR
        # Carve path to core
        grid.grid[5, :, CORE_Z] = VOXEL_AIR
        grid.grid[:, 5, CORE_Z] = VOXEL_AIR
        grid.grid[5, 5, CORE_Z] = VOXEL_AIR
    ...
    def test_underworld_spawned_flag(self):
        ai, bus, grid, core, rng = _make_ai()
        ai.spawning_enabled = True
        # Carve air at edges (z=17 is within underworld spawn range)
        for x in range(grid.width):
            grid.grid[x, 0, 17] = VOXEL_AIR
        for y in range(grid.depth):
            grid.grid[0, y, 17] = VOXEL_AIR
    ...
    def test_caps_independent(self):
        ...
        for n in range(30):
            si = Intruder(
                intruder_id=100 + n, x=0, y=0, z=SURFACE_Z,
...
class TestUnderworldRetreat:
    ...
    def test_is_at_deep_edge_true(self):
        ai, bus, grid, core, rng = _make_ai()
        intruder = _make_intruder(x=0, y=5, z=17)
        assert ai._is_at_deep_edge(intruder) is True

    def test_is_at_deep_edge_false_surface(self):
        ai, bus, grid, core, rng = _make_ai()
        intruder = _make_intruder(x=0, y=5, z=SURFACE_Z)
        assert ai._is_at_deep_edge(intruder) is False

    def test_is_at_deep_edge_false_interior(self):
        ai, bus, grid, core, rng = _make_ai()
        intruder = _make_intruder(x=5, y=5, z=17)
        assert ai._is_at_deep_edge(intruder) is False

    def test_is_at_deep_edge_all_edges(self):
        ai, bus, grid, core, rng = _make_ai()
        # x=0
        assert ai._is_at_deep_edge(_make_intruder(x=0, y=3, z=20)) is True
        # x=width-1
        assert ai._is_at_deep_edge(
            _make_intruder(x=grid.width - 1, y=3, z=20)
        ) is True
        # y=0
        assert ai._is_at_deep_edge(_make_intruder(x=3, y=0, z=20)) is True
        # y=depth-1
        assert ai._is_at_deep_edge(
            _make_intruder(x=3, y=grid.depth - 1, z=20)
        ) is True

    def test_underworlder_escapes_at_deep_edge(self):
        ai, bus, grid, core, rng = _make_ai()
        intruder = _make_intruder(
            arch=TREMORSTALKER, x=0, y=5, z=17,
        )
    ...
    def test_surface_intruder_does_not_escape_at_deep_edge(self):
        ...
        ai.intruders.append(intruder)
        ai._update_retreating(intruder)
        # NOT escaped — surface intruder needs to reach z=SURFACE_Z
        assert intruder.state != IntruderState.ESCAPED
    ...
    def test_start_retreat_underworlder_no_exit(self):
        ...
        intruder = _make_intruder(
            arch=TREMORSTALKER, x=5, y=5, z=17,
        )
        intruder.state = IntruderState.ADVANCING
        ai._start_retreat(intruder)
...
class TestBoremiteDigging:
    ...
    def test_dig_duration_one_third(self):
        ai, bus, grid, core, rng = _make_ai()
        intruder = _make_intruder(arch=BOREMITE, x=3, y=3, z=CORE_Z)
        intruder.state = IntruderState.ADVANCING
        ai.intruders.append(intruder)
    ...
    def test_dig_faster_than_tunneler(self):
        ai, bus, grid, core, rng = _make_ai()
        boremite = _make_intruder(arch=BOREMITE, x=3, y=3, z=CORE_Z)
        from dungeon_builder.intruders.archetypes import TUNNELER
        tunneler = _make_intruder(
            arch=TUNNELER, x=3, y=3, z=CORE_Z,
...
class TestCorrosiveCrawler:
    ...
    def test_corrosive_adds_stress_after_dig(self):
        ...
        intruder = _make_intruder(arch=CORROSIVE_CRAWLER, x=3, y=3, z=CORE_Z)
        intruder.state = IntruderState.INTERACTING
        intruder.interaction_type = "dig"
        intruder.interaction_target = (4, 3, CORE_Z)
        intruder.interaction_ticks = 1  # Will complete this tick
        ai.intruders.append(intruder)

        # Place stone at target and neighbors
        grid.grid[4, 3, CORE_Z] = VOXEL_STONE
        grid.grid[5, 3, CORE_Z] = VOXEL_STONE
        grid.grid[3, 3, CORE_Z] = VOXEL_STONE
        grid.grid[4, 4, CORE_Z] = VOXEL_STONE

        # Initial stress should be 0
        assert grid.stress_ratio[5, 3, CORE_Z] == 0.0

        ai._update_interacting(intruder)

        # Neighbors should have increased stress_ratio
        assert grid.stress_ratio[5, 3, CORE_Z] == pytest.approx(CORROSIVE_DAMAGE_FACTOR)
        # The block at (3,3,CORE_Z) — the intruder's own position (solid) — also affected
        assert grid.stress_ratio[3, 3, CORE_Z] == pytest.approx(CORROSIVE_DAMAGE_FACTOR)
        assert grid.stress_ratio[4, 4, CORE_Z] == pytest.approx(CORROSIVE_DAMAGE_FACTOR)
    ...
    def test_corrosive_only_affects_solid(self):
        ...
        grid.grid[4, 3, CORE_Z] = VOXEL_STONE
        grid.grid[5, 3, CORE_Z] = VOXEL_AIR  # air neighbor
        grid.grid[3, 3, CORE_Z] = VOXEL_STONE  # solid neighbor

        ai._apply_corrosive_damage(4, 3, CORE_Z)
...
class TestMagmawraithHeat:
    ...
    def test_heats_adjacent_blocks(self):
        ai, bus, grid, core, rng = _make_ai()
        intruder = _make_intruder(arch=MAGMAWRAITH, x=5, y=5, z=CORE_Z)
        intruder.state = IntruderState.ADVANCING
        ai.intruders.append(intruder)

        initial_temp = float(grid.temperature[6, 5, CORE_Z])
        ai._tick_magmawraith_heat(intruder, MAGMAWRAITH_HEAT_INTERVAL)
        assert grid.temperature[6, 5, CORE_Z] == pytest.approx(
    ...
    def test_no_heat_off_interval(self):
        ...
        assert grid.temperature[6, 5, CORE_Z] == pytest.approx(initial_temp)

    def test_no_heat_for_non_magmawraith(self):
        ai, bus, grid, core, rng = _make_ai()
        intruder = _make_intruder(arch=BOREMITE, x=5, y=5, z=CORE_Z)
        initial_temp = float(grid.temperature[6, 5, CORE_Z])
        ai._tick_magmawraith_heat(intruder, MAGMAWRAITH_HEAT_INTERVAL)
        assert grid.temperature[6, 5, CORE_Z] == pytest.approx(initial_temp)
