...
class TestGradientProducesFatigue:
    def test_gradient_produces_fatigue(self):
        ...
        # Compute a temperature gradient that puts ratio above the 0.1
        # decay threshold so fatigue actually accumulates.
        # ratio = gradient * CTE / tensile > 0.1
        # gradient > 0.1 * tensile / CTE (with comfortable margin)
        stone_cte = VOXEL_CTE[VOXEL_STONE]
        stone_tensile = VOXEL_TENSILE_STRENGTH[VOXEL_STONE]
        min_gradient = 0.15 * stone_tensile / stone_cte  # 50% above threshold
        hot_temp = max(min_gradient, 500.0)

        grid.temperature[:, :, :] = 0.0
        grid.temperature[3, 3, 2] = hot_temp
...
class TestObsidianCracksNearLava:
    def test_obsidian_cracks_near_lava(self):
        ...
        # Compute how many ticks until crack threshold
        obs_cte = VOXEL_CTE[VOXEL_OBSIDIAN]
        obs_tensile = VOXEL_TENSILE_STRENGTH[VOXEL_OBSIDIAN]
        ratio = LAVA_TEMPERATURE * obs_cte / obs_tensile
        delta = ratio * THERMAL_FATIGUE_ACCUMULATION
        ticks_needed = int(THERMAL_CRACK_THRESHOLD / delta) + 3  # +3 margin
        ...
        for i in range(1, ticks_needed + 1):
...
class TestGraniteResistsThermalShock:
    def test_granite_resists_thermal_shock(self):
        ...
        granite_cte = VOXEL_CTE[VOXEL_GRANITE]
        granite_tensile = VOXEL_TENSILE_STRENGTH[VOXEL_GRANITE]
        ratio = LAVA_TEMPERATURE * granite_cte / granite_tensile
        # Granite has very low ratio. If ratio < 0.1, fatigue decays rather
        # than accumulating, so we just verify it doesn't crack after 5 ticks.
        # If ratio >= 0.1, we also verify fatigue accumulates.
        ...
        # After 5 ticks, should NOT be cracked (granite is resilient)
        for i in range(1, 6):
            _tick(bus, THERMAL_STRESS_TICK_INTERVAL * i)

        assert bool(grid.loose[3, 3, 2]) is False
        # If ratio is above decay threshold, fatigue should accumulate
        if ratio >= 0.1:
            assert grid.thermal_fatigue[3, 3, 2] > 0.0
...
class TestMetalResistsThermalStress:
    def test_metal_resists_thermal_stress(self):
        ...
        # Iron has very low CTE and high tensile → ratio is tiny
...
class TestFatigueDecays:
    def test_fatigue_decays_when_cool(self):
        ...
        # Compute gradient that produces fatigue above decay threshold
        stone_cte = VOXEL_CTE[VOXEL_STONE]
        stone_tensile = VOXEL_TENSILE_STRENGTH[VOXEL_STONE]
        min_gradient = 0.15 * stone_tensile / stone_cte
        hot_temp = max(min_gradient, 500.0)
...
class TestCrackedBlocksBecomeLoose:
    def test_cracked_blocks_become_loose(self):
        ...
        # Compute fatigue delta per tick from lava-temp gradient on obsidian
        obs_cte = VOXEL_CTE[VOXEL_OBSIDIAN]
        obs_tensile = VOXEL_TENSILE_STRENGTH[VOXEL_OBSIDIAN]
        ratio = LAVA_TEMPERATURE * obs_cte / obs_tensile
        delta = ratio * THERMAL_FATIGUE_ACCUMULATION

        # Set fatigue just below threshold, close enough that one tick pushes over
        grid.thermal_fatigue[3, 3, 2] = THERMAL_CRACK_THRESHOLD - delta * 0.5
...
class TestOnlyRunsOnInterval:
    def test_only_runs_on_interval(self):
        ...
        # Ensure gradient produces ratio > 0.1 to overcome decay
        stone_cte = VOXEL_CTE[VOXEL_STONE]
        stone_tensile = VOXEL_TENSILE_STRENGTH[VOXEL_STONE]
        min_gradient = 0.15 * stone_tensile / stone_cte
        hot_temp = max(min_gradient, 500.0)
...
class TestThermalCrackResetsFatigue:
    def test_thermal_crack_resets_fatigue(self):
        ...
        # Compute per-tick delta for obsidian at lava temperature
        obs_cte = VOXEL_CTE[VOXEL_OBSIDIAN]
        obs_tensile = VOXEL_TENSILE_STRENGTH[VOXEL_OBSIDIAN]
        ratio = LAVA_TEMPERATURE * obs_cte / obs_tensile
        delta = ratio * THERMAL_FATIGUE_ACCUMULATION

        # Set fatigue so one more tick pushes past threshold
        grid.thermal_fatigue[3, 3, 2] = THERMAL_CRACK_THRESHOLD - delta * 0.5
        grid.grid[4, 3, 2] = VOXEL_STONE
        grid.temperature[4, 3, 2] = LAVA_TEMPERATURE
        grid.temperature[3, 3, 2] = 0.0
...
class TestObsidianQuenchCracksFast:
    def test_obsidian_quench_cracks_fast(self):
        """Obsidian at high temp next to water cracks quickly via quenching."""
        bus, grid, phys = _setup()
        grid.grid[:, :, :] = VOXEL_AIR
        grid.grid[3, 3, 2] = VOXEL_OBSIDIAN
        grid.grid[3, 3, 3] = VOXEL_BEDROCK
        ...
        grid.temperature[2, 3, 2] = WATER_TEMPERATURE

        # Compute ticks needed with quench multiplier
        gradient = 800.0 - WATER_TEMPERATURE
        obs_cte = VOXEL_CTE[VOXEL_OBSIDIAN]
        obs_tensile = VOXEL_TENSILE_STRENGTH[VOXEL_OBSIDIAN]
        stress = gradient * obs_cte * QUENCH_MULTIPLIER
        ratio = stress / obs_tensile
        delta = ratio * THERMAL_FATIGUE_ACCUMULATION
        ticks_needed = int(THERMAL_CRACK_THRESHOLD / delta) + 3
